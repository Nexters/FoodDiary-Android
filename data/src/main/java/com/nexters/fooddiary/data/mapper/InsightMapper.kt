...
class InsightMapper @Inject constructor() {
    fun toDomain(response: InsightsResponse): UserInsights {
        return UserInsights(
            month = response.month,
            photoStats = response.photoStats.toDomain(),
            categoryStats = response.categoryStats.toDomain(),
            weeklyStats = response.weeklyStats.toDomain(),
            diaryTimeStats = response.diaryTimeStats.toDomain(),
            tagStats = response.tagStats.map { it.toDomain() },
            locationStats = response.locationStats.map { it.toDomain() },
        )
    }
